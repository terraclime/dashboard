...
$stackName = "terraclime-profile"
$region = if ([string]::IsNullOrWhiteSpace($env:AWS_REGION)) { "ap-south-1" } else { $env:AWS_REGION }
$artifactBucket = if ([string]::IsNullOrWhiteSpace($env:PROFILE_ARTIFACT_BUCKET)) { "terraclime-data-live-artifacts-717279694116-ap-south-1" } else { $env:PROFILE_ARTIFACT_BUCKET }
$artifactPrefix = if ([string]::IsNullOrWhiteSpace($env:PROFILE_ARTIFACT_PREFIX)) { "terraclime-profile" } else { $env:PROFILE_ARTIFACT_PREFIX }
$packagedTemplate = "packaged.yaml"

$customDomainName = if ([string]::IsNullOrWhiteSpace($env:PROFILE_DOMAIN_NAME)) { "profile.terraclime.com" } else { $env:PROFILE_DOMAIN_NAME }
$certificateArn = $env:PROFILE_CERTIFICATE_ARN
...
if (-not [string]::IsNullOrWhiteSpace($customDomainName)) {
  if ([string]::IsNullOrWhiteSpace($certificateArn)) {
    ...
    if ($null -eq $matchingCertificate) {
      Write-Error "No issued ACM certificate found for $customDomainName in $region. Set PROFILE_CERTIFICATE_ARN and rerun."
    }
