...
describe("Demo service layer", () => {
  ...
  it("returns the correct settings context for standard and prepaid users", async () => {
    const standardProfile = await getProfile("demo@terraclime.com");
    assert.equal(standardProfile.user.account_type, "standard");
    assert.equal(standardProfile.apartment.id, "SOBHA-TWR-1");

    const prepaidProfile = await getProfile("prepaid@terraclime.com");
    assert.equal(prepaidProfile.user.account_type, "prepaid");
    assert.equal(prepaidProfile.apartment.id, "KRM-ZONE-1");
    assert.equal(prepaidProfile.apartment.name, "Koramangala Urban Water Zone");
  });
